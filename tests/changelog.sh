...
tmp=$(mktemp -d "${TMPDIR:-/tmp}/ember-changelog.XXXXXX")
trap 'rm -rf "$tmp"' EXIT
...
export GIT_AUTHOR_NAME=ember-test GIT_AUTHOR_EMAIL=ember-test@example.com
export GIT_COMMITTER_NAME=ember-test GIT_COMMITTER_EMAIL=ember-test@example.com
